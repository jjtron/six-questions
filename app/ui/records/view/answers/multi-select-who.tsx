...
export default function MultiSelectWho(props:  {options: SelectOptions, initSelected: number[] } ) {
    ...
    return (
        <div className="overflow-auto flex flex-col h-[140px] max-[431px]:h-[170px] w-full px-2">
        ...
        definedStoredList.forEach((storedEl: any, j) => {
            isCheckedArray.push(
                <div key={j} className="flex flex-row">
                    <div className={clsx(`max-[431px]:basis-1/2 basis-1/5 text-sm bg-green-100 
                                          border-tl-1 border-slate-400 px-2 `,
                                          { "rounded-tl-md" : j === 0,
                                            "rounded-bl-md border-tlb-1" : j === definedStoredList.length - 1
                                          })}>{storedEl[1].name}:</div>
                    <div className={clsx(`max-[431px]:basis-1/2 basis-4/5 text-xs bg-white 
