...
export function PlacesTable({records} : {records : any} ) {
  ...
  function handleClick(selectedRecordId: number) {
      ...
      { records.map((group: any, i: number) => {
            ...
            return (
              group.map((record: any, j: number) => {
                return (
                  <div key={group.length * i + j} className="w-full flex flex-col">
                    ...
                    <div className={clsx("flex flex-row rounded-md", { "bg-sky-300" : (group.length * i + j) === rowIndex, "bg-sky-200" : (group.length * i + j) !== rowIndex })}
                        onClick={() => handleClick(group.length * i + j)} >
                      <div className="basis-1/4 pl-2 border-tbl-1 border-slate-400 rounded-l-md">{record.name}{group.length * i + j}</div>
